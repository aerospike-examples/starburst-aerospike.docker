...
export TABLE_DESC_DIR=${TABLE_DESC_DIR:-/etc/starburst/aerospike}
...
if [ -f /tmp/aerospike.properties.template ] && [ $AEROSPIKE_ENVS -eq 1 ]; then
  envsubst < /tmp/aerospike.properties.template > /etc/starburst/catalog/aerospike.properties
fi

export TRINO_NODE_ID=$(uuidgen)
echo "TRINO_NODE_ID=$TRINO_NODE_ID"
envsubst < /tmp/trino.node.properties.template > /etc/starburst/node.properties
...
if [[ -z "${TRINO_NODE_TYPE}" ]]; then
    ...
    envsubst < /tmp/coordinator.config.properties.template > /etc/starburst/config.properties
elif [[ $TRINO_NODE_TYPE == "worker" ]]; then
    echo "Configuring a worker Trino node"
    envsubst < /tmp/worker.config.properties.template > /etc/starburst/config.properties
else 
    printf '%s\n' "Invalid TRINO_NODE_TYPE parameter: $TRINO_NODE_TYPE" >&2
    exit 1
fi

chown -R starburst:root /etc/starburst
mkdir /data/trino

# remove unused plugins
cd /usr/lib/starburst/plugin 
ls | grep -v 'aerospike\|memory\|jmx\|system\|tpcds\|tpch' | xargs rm -rf
ls -la /usr/lib/starburst/plugin
cd /
